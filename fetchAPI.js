...
const MINE_LP = 'terra1kcthelkax4j9x8d3ny6sdag0qmxxynl3qtcrpy'
...
exports.sendMessage = function (TELEGRAM_BOT_URL, CHAT_ID, TEXT_MESSAGE) {
...
exports.MINE_LP_staking_amount = async function(walletAdd){
    let staking = await getAPI('wasm/contracts/' + MINE_token + '/store?query_msg={"staker_info":{"staker":"' + walletAdd + '"}}') 
    return parseInt(staking.result.bond_amount)
}

exports.MINE_LP_balance = async function(walletAdd){
    let balance = await getAPI('wasm/contracts/' + MINE_LP + '/store?query_msg={"balance":{"address":"' + walletAdd +'"}}')
    console.log(balance.result)
    let LP_Blance = parseInt(balance.result.balance)
    return LP_Blance
}

exports.MINE_USTperLP = async function(){
    let pool = await getAPI('wasm/contracts/' + MINE_pool + '/store?query_msg={"pool":{}}')
