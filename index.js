const TOKEN = '1846034805:AAGonEXVDfv1niOq7uj1glzoeZLZJH1ZPB8';
// 텔레그램 @BotFather 로 생성한 봇의 TOKEN이 필요합니다.
//, "MIR LP","mGOOGL LP", "aUST" 
...
const MINE_pool = 'terra178jydtjvj4gw8earkgnqc80c3hrmqj4kw2welz'
const MINE_LP = 'terra1rqkyau9hanxtn63mjrdfhpnkpddztv3qav0tq2'
const MINE_token = 'terra19nek85kaqrvzlxygw20jhy08h3ryjf5kg4ep3l'    
...
class Repay{
    ...
    //mine lp withdraw
    //1. unstake lp token
    async unstake_MINE_LP(LP_token_amount){
        console.log('Unstaking MINE-UST LP Token...')
    ...
    //2. withdraw lp
    async withdraw_MINE_LP(LP_token_amount){
        console.log('Withdrawing MINE-UST LP...')
        let withdraw = new MsgExecuteContract(
            this.wallet.key.accAddress,
            MINE_LP,
            {
...
async function MINE_LP_process(insufficientUST){
    let LP_staking = await fetchAPI.MINE_LP_staking_amount(myAddress)
    let LP_Balance = await fetchAPI.MINE_LP_balance(myAddress)
    let USTperLP = await fetchAPI.MINE_USTperLP()
    let LP_needed = parseInt(insufficientUST/USTperLP)
    let LP_unstake_amount = 0
    let LP_withdraw_amount = 0
    if (insufficientUST > (LP_staking + LP_Balance)*USTperLP){ //in this case withdraw all
    ...
    if (LP_unstake_amount != 0){
        await repayHandler.unstake_MINE_LP(LP_unstake_amount)
    }
    if (LP_withdraw_amount !=0){
        await repayHandler.withdraw_MINE_LP(LP_withdraw_amount)
    }
}
...
async function get_UST(option, insufficientUST){
    ...
    if (option[0] == 'aUST'){
        ...
        if(option[1]=='MINE'){
            await MINE_LP_process(insufficientUST)
        }else if(option[1]=='ANC'){
...
async function update_state(){
    ...
    if(percentNow > 0.945){
        fetchAPI.sendMessage(TELEGRAM_BOT_URL, CHAT_ID, "94.5 넘었다");
    }
...
async function main(){
    ...
    if(option_check && UST_check){
        while(true){
            var test = await fetchAPI.MINE_USTperLP()
            console.log(test);
            // var test = 1000;
            // repayHandler.withdraw_MINE_LP(test);
            // console.log(test);
            ...
            if(nowPercent > trigger_percent){
                ...
                if (UST_remain < total_needed_amount){
                    fetchAPI.sendMessage(TELEGRAM_BOT_URL, CHAT_ID, "있는 돈:" + UST_remain + "  갚아야될 돈" + total_needed_amount + "get-ust 프로세스 진행 중");
